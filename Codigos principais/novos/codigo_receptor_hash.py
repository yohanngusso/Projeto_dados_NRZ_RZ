...
def rz_decoding(signal):
    ...
    for i in range(0, len(signal), 4):
        if signal[i:i + 2] == [-1, -1] and signal[i + 2:i + 4] == [0, 0]:
            binary_message += '0'
        elif signal[i:i + 2] == [1, 1] and signal[i + 2:i + 4] == [0, 0]:
            binary_message += '1'
    return binary_message


def about(nrzcoded_message,nrzIcoded_message, rzcoded_message, key):
    # Decodifica NRZ
    binary_message_nrz = nrz_decoding(nrzcoded_message)
    # Decodifica NRZ-I
    binary_message_nrzI = nrzI_decoding(nrzIcoded_message)
    # Decodifica RZ
    binary_message_rz = rz_decoding(rzcoded_message)
    # Converte binário para texto
    decrypted_message_nrz = binary_to_text(binary_message_nrz)
    decrypted_message_nrzI = binary_to_text(binary_message_nrzI)
    decrypted_message_rz = binary_to_text(binary_message_rz)
    #Descrptado
    decrypted_nrz = decrypt(decrypted_message_nrz, key)
    decrypted_nrzI = decrypt(decrypted_message_nrzI, key)
    decrypted_rz = decrypt(decrypted_message_rz, key)

    return decrypted_nrz,decrypted_nrzI,decrypted_rz
...
class OutputWindow(QtWidgets.QDialog):
    def __init__(self, decrypted_nrz,decrypted_nrzI, decrypted_rz):
        ...
        self.text_edit.append("Binario message nrz: {}".format(decrypted_nrz))
        self.text_edit.append("\n")
        self.text_edit.append("Binario message nrz: {}".format(decrypted_nrzI))
        self.text_edit.append("\n")
        self.text_edit.append("mensagem_codificada_nrzI: {}".format(decrypted_rz))
...
class Ui_MainWindow(object):
    ...
    def receber_button_clicked(self):
        HOST = '0.0.0.0'  # Endereço IP do emissor (deixe como '0.0.0.0' para aceitar conexões de qualquer IP)
        ...
        mensagem_codificada_nrz = data_nrz.decode() # decode -> converte sequencia de bytes em uma string legível
         # Recebe a mensagem codificada NRZ
        data_nrzI = conn.recv(1024)
        mensagem_codificada_nrzI = data_nrzI.decode()
        # Recebe a mensagem codificada RZ 
        data_rz = conn.recv(1024)
        mensagem_codificada_rz = data_rz.decode() 

        # # Converter mensagem_codificada_nrz em uma lista de inteiros
        # lista_inteiros_nrz = [int(bit) if bit != '-' else -1 for bit in mensagem_codificada_nrz]

        # # Converter mensagem_codificada_rz em uma lista de inteiros
        # lista_inteiros_rz = [int(bit) if bit != '-' else -1 for bit in mensagem_codificada_rz]

        # Chama a função 'about' para decodificar e descriptografar as mensagens
        key_input, ok = QtWidgets.QInputDialog.getText(MainWindow, "Digite a Chave", "Chave:", QtWidgets.QLineEdit.Normal)
        if ok and key_input:
            # Utilize a chave digitada pelo usuário aqui
            print("Chave digitada:", key_input)

            # Ajusta o tamanho da chave para 32 bytes usando uma função de hash
